...
class NatOrderedEvidence : Ordered.Evidence {
    override fun orderReflexiveEvidence(a: Expr) = proved {
        ...
        applyByMpChain(leqIntroduction, ex)
    }

    override fun orderTransitiveEvidence(a: Expr, b: Expr, c: Expr) = proved {
        assume(a leq b) { leqAB ->
            val exAB = applyByMpChain(leqElimination(a, b), leqAB)
            assume(b leq c) { leqBC ->
                val exBC = applyByMpChain(leqElimination(b, c), leqBC)
                eliminateExists(exAB, "x") { x, ab -> // a + x == b
                    eliminateExists(exBC, "y") { y, bc -> // b + y == c
                        ...
                        applyByMpChain(leqIntroduction, exAC) // a <= c
                    }
                }
            }
        }
    ...
    override fun orderAntisymmetricEvidence(a: Expr, b: Expr) = proved {
        assume(a leq b) { leqAB ->
            val exAB = applyByMpChain(leqElimination, leqAB)
            assume(b leq a) { leqBA ->
                val exBA = applyByMpChain(leqElimination, leqBA)
                eliminateExists(exAB, "x") { x, ab -> // a + x == b
                    eliminateExists(exBA, "y") { y, ba -> // b + y == a
                        ...
                        val xyZero = applyByMpChain(sumEqualsLeftImpliesRightZero, aXyEqA) // x + y == 0
                        val xAndYZero = applyByMpChain(sumZeroImpliesBothZero, xyZero) // x == 0 && y == 0
                        val xZero = applyByMpChain(LogicAxioms.andEliminationLeft, xAndYZero) // x == 0
    ...
    override fun orderTotalEvidence(a: Expr, b: Expr) = proved {
        val base = forAllByGeneralization("x", Nat) { x ->
            ...
            val zeroX = applyByMpChain(EqualityLibrary.transitivity, bZeroRevEq, bZero) // 0 + x == x
            val exT = zeroX.introduceExists(x, Occurences.First)
            val leqX = applyByMpChain(leqIntroduction, exT)
            applyByMpChain(LogicAxioms.orIntroductionLeft(auto(), x leq zero), leqX)
        }
        ...
        val step = forAllByGeneralization("k", Nat) { k ->
            assume(forall("x", Nat) { x -> (k leq x) or (x leq k) }) { forallK ->
                val subBase = run {
                    ...
                    val leqSK = applyByMpChain(leqIntroduction, exT)
                    applyByMpChain(LogicAxioms.orIntroductionRight(succ(k) leq zero, auto()), leqSK)
                }
                ...
                val subStep = forAllByGeneralization("m", Nat) { m ->
                    assume((succ(k) leq m) or (m leq succ(k))) { _ ->
                        ...
                        val ifKM = assume(k leq m) { km -> // k <= m
                            val exKM = applyByMpChain(leqElimination, km)
                            eliminateExists(exKM, "y") { y, kym -> // k + y == m
                                ...
                                val sksmLeq = applyByMpChain(leqIntroduction, exT) // S(k) <= S(m)
                                applyByMpChain(LogicAxioms.orIntroductionLeft(
                                    auto(), succ(m) leq succ(k)
                                ), sksmLeq) // S(k) <= S(m) or S(m) <= S(k)
                            }
                        }
                        val ifMK = assume(m leq k) { mk -> // m <= k
                            val exMK = applyByMpChain(leqElimination, mk)
                            eliminateExists(exMK, "z") { z, myk -> // m + z == k
                                ...
                                val sksmLeq = applyByMpChain(leqIntroduction, exT) // S(m) <= S(k)
                                applyByMpChain(LogicAxioms.orIntroductionRight(
                                    succ(k) leq succ(m), auto()
                                ), sksmLeq) // S(k) <= S(m) or S(m) <= S(k)

                            }
                        }
                        applyByMpChain(LogicAxioms.orElimination, ifKM, ifMK, kmOr)
                    }
                }

                applyByMpChain(NatAxioms.induction, subBase, subStep)
            }
        }

        val forallUV = applyByMpChain(NatAxioms.induction, base, step)
    ...
    override fun addPreservesOrderRightEvidence(x: Expr, y: Expr, z: Expr) = proved {
        assume(x leq y) { leqXY ->
            val exXY = applyByMpChain(leqElimination, leqXY) // exists k. x + k == y
            eliminateExists(exXY, "k") { k, xkyEq -> // x + k == y
                ...
                applyByMpChain(leqIntroduction, step4)
            }
        }
    }
    ...
    override fun mulPreservesNonNegativeEvidence(a: Expr, b: Expr) = proved {
        ...
        val bMulLeq = applyByMpChain(LogicAxioms.hilbertAxiom1(auto(), zero leq b), mulLeq) // 0 <= b -> 0 <= a * b
        applyByMpChain(LogicAxioms.hilbertAxiom1(auto(), zero leq a), bMulLeq) // 0 <= a -> 0 <= b -> 0 <= a * b
    }
}
