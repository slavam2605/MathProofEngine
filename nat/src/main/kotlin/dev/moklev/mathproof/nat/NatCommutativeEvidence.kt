...
class NatCommutativeEvidence : Commutative.Evidence {
    override fun mulCommutativeEvidence(a: Expr, b: Expr): ProofEvidence = proved {
        val base = run {
            ...
            val step3 = applyByMpChain(EqualityLibrary.symmetry, step2) // 0 == 0 * a
            applyByMpChain(EqualityLibrary.transitivity, step1, step3) // a * 0 == 0 * a
        }
        ...
        val step = forAllByGeneralization("x", Nat) { x ->
            assume(a * x eq x * a) { axXA -> // a * x == x * a
                ...
                val step4 = applyByMpChain(EqualityLibrary.symmetry, step3) // a + x * a == S(x) * a
                val step5 = applyByMpChain(EqualityLibrary.transitivity, step1, step2) // a * S(x) == a + x * a
                applyByMpChain(EqualityLibrary.transitivity, step5, step4) // a * S(x) == S(x) * a
            }
        }
