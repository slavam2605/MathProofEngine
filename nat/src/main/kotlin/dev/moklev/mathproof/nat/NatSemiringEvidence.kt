...
class NatSemiringEvidence : SemiringTheory.Evidence {
    override fun addAssociativeEvidence(x: Expr, y: Expr, z: Expr) = proved {
        val base = run {
            ...
            val step3 = applyByMpChain(EqualityLibrary.symmetry, step2) // y == y + 0
            val step4 = applyByMpChain(EqualityLibrary.congruence(
                lambda("t", Nat) { t -> x + t }, auto(), auto()
            ), step3) // x + y == x + (y + 0)
            applyByMpChain(EqualityLibrary.transitivity, step1, step4)
        }
        ...
        val step = forAllByGeneralization("u", Nat) { u ->
            assume((x + y) + u eq x + (y + u)) { prevStep -> // (x + y) + u == x + (y + u)
                ...
                val step2 = applyByMpChain(EqualityLibrary.congruence(
                    Succ, auto(), auto()
                ), prevStep) // S((x + y) + u) == S(x + (y + u))
                val step3 = applyByMpChain(EqualityLibrary.transitivity, step1, step2) // (x + y) + S(u) == S(x + (y + u))
                val step4 = applyByMpChain(EqualityLibrary.symmetry,
                    infer(NatAxioms.addDefinitionSucc(x, y + u))) // S(x + (y + u)) == x + S(y + u)
                val step5 = applyByMpChain(EqualityLibrary.transitivity, step3, step4) // (x + y) + S(u) == x + S(y + u)
                val step6 = applyByMpChain(EqualityLibrary.symmetry,
                    infer(NatAxioms.addDefinitionSucc(y, u))) // S(y + u) == y + S(u)
                val step7 = applyByMpChain(EqualityLibrary.congruence(
                    lambda("t", Nat) { t -> x + t }, auto(), auto()
                ), step6) // x + S(y + u) == x + (y + S(u))
                applyByMpChain(EqualityLibrary.transitivity, step5, step7)
            }
        }

        applyByMpChain(NatAxioms.induction, base, step).instantiateForall(z)
    }
    ...
    override fun addCommutativeEvidence(x: Expr, y: Expr) = proved {
        val base = run {
            ...
            val step3 = applyByMpChain(EqualityLibrary.symmetry, step2) // x == 0 + x
            applyByMpChain(EqualityLibrary.transitivity, step1, step3) // x + 0 == 0 + x
        }
        ...
        val step = forAllByGeneralization("z", Nat) { z ->
            assume((x + z) eq (z + x)) { prevStep -> // x + z == z + x
                ...
                val step3 = applyByMpChain(EqualityLibrary.congruence(
                    Succ, auto(), auto()
                ), prevStep) // S(x + z) == S(z + x)
                val step4 = applyByMpChain(EqualityLibrary.transitivity, step1, step3) // x + S(z) == S(z + x)
                val step5 = applyByMpChain(EqualityLibrary.symmetry, step2) // S(z + x) == S(z) + x
                applyByMpChain(EqualityLibrary.transitivity, step4, step5) // x + S(z) == S(z) + x
            }
        }

        applyByMpChain(NatAxioms.induction, base, step).instantiateForall(y)
    }
    ...
    override fun mulAssociativeEvidence(x: Expr, y: Expr, z: Expr) = proved {
        ...
        val step = forAllByGeneralization("u", Nat) { u ->
            assume((x * y) * u eq x * (y * u)) { prevStep -> // (x * y) * u == x * (y * u)
                ...
                val step4 = applyByMpChain(EqualityLibrary.transitivity, step2, step3) // (x * y) * S(u) == x * (y + y * u)
    ...
    override fun mulZeroLeftEvidence(x: Expr) = proved {
        ...
        applyByMpChain(NatAxioms.induction, base, step).instantiateForall(x)
    }
    ...
    override fun mulOneRightEvidence(x: Expr) = proved {
        ...
        val step5 = applyByMpChain(EqualityLibrary.transitivity, step1, step3) // x * S(0) == x + 0
        applyByMpChain(EqualityLibrary.transitivity, step5, step4) // x * S(0) == x
    }
    ...
    override fun mulDistributesOverAddLeftEvidence(x: Expr, y: Expr, z: Expr) = proved {
        ...
        val step5 = applyByMpChain(EqualityLibrary.transitivity, step3, step4) // x * (y + z) == (z + y) * x
        val step6 = infer(NatTheory.mulDistributesOverAddRight(z, y, x)) // (z + y) * x == z * x + y * x
        val step7 = applyByMpChain(EqualityLibrary.transitivity, step5, step6) // x * (y + z) == z * x + y * x
